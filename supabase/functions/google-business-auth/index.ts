...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const CLIENT_ID = Deno.env.get('GOOGLE_BUSINESS_PROFILE_CLIENT_ID');
    const CLIENT_SECRET = Deno.env.get('GOOGLE_BUSINESS_PROFILE_CLIENT_SECRET');
    ...
    const response = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        ...
        redirect_uri: redirectUri || 'http://localhost:5173/google-business/callback',
    ...
    if (!response.ok) {
      console.error('Google Business OAuth error:', data);
